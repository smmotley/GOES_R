...
def main():
    ...
    for FILE in (FILE for FILE in ALL_FILES.keys() if FILE not in already_examined):
        ...
        sql = '''INSERT INTO fdfc_files_downloaded 
...
def alert_user(alert_num, userID, st_time, fire_id):
    ...
    for idx, row in df.iterrows():
        if row['need_to_alert']:
            if row['alerted_incident_id'] is not None:
                ...
                sql_update = "UPDATE user_alert_log SET need_to_alert=FALSE WHERE user_id=? AND alerted_incident_id=?"
                CURSOR.execute(sql_update, [userID, row['alerted_incident_id']])
                CONN.commit()
            if row['alerted_cal_fire_incident_id'] is not None:
                sql = "SELECT * FROM cal_fire WHERE incident_id = ?"
                ...
                sql_update = "UPDATE user_alert_log SET need_to_alert=0 " \
    ...
    sql_update = "SELECT * FROM user_alert_log WHERE need_to_alert=1 " \
                 "AND user_id=?"
...
def update_alertDB(loop_duration, user_email):
    ...
    sql = "SELECT user_lng, user_lat, user_id FROM users WHERE email == ?"
...
def append_db(data, conn):
    try:
        data.to_sql('goes_r_fire', conn, index=True, if_exists='append')
        return 'Success'
    except Exception as e:
        print("Initial failure to append: {}\n".format(e))
        print("Attempting to rectify...")
        existing = pd.read_sql('SELECT * FROM goes_r_fire', con=conn)
        to_insert = data.reset_index().rename(columns={'index': 'id'})
        mask = ~to_insert.id.isin(existing.id)
        try:
            to_insert.loc[mask].to_sql('goes_r_fire', con=conn, index=False, if_exists='append')
